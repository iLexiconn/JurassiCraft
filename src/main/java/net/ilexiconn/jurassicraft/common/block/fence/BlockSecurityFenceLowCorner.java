...
public class BlockSecurityFenceLowCorner extends BlockSecurityFence {
    public BlockSecurityFenceLowCorner() {
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int metadata) {
        ...
        if (tileEntity instanceof TileSecurityFenceLowCorner) {
            TileSecurityFenceLowCorner fence = (TileSecurityFenceLowCorner) tileEntity;
            ...
            if (fence.hasItems() && !world.isRemote) {
                ...
                for (int side = 0; side < 4; side++) {
                    if (fence.hasFenceAt(side)) {
                        TileSecurityFenceLowCorner neighborFence = fence.getNextLowSecurityMainFenceBlock(fence, side, 128);
    ...
    public TileEntity createNewTileEntity(World world, int metadata) {
        return new TileSecurityFenceLowCorner();
    }
}
