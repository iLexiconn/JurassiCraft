...
public class TileSecurityFenceHighCorner extends TileEntity implements ISidedInventory {
    ...
    private boolean hasHighSecurityCornerFenceBlockAt(TileSecurityFenceHighCorner mainFence, int x, int y, int z) {
        return mainFence.worldObj.getBlock(x, y, z) instanceof BlockSecurityFenceHighCorner;
    }

    private boolean hasHighSecurityBaseFenceBlockAt(TileSecurityFenceHighCorner mainFence, int x, int y, int z) {
        return mainFence.worldObj.getBlock(x, y, z) instanceof BlockSecurityFenceHighBase;
    }

    private boolean hasHighSecurityPoleFenceBlockAt(TileSecurityFenceHighCorner mainFence, int x, int y, int z) {
        return mainFence.worldObj.getBlock(x, y, z) instanceof BlockSecurityFenceHighPole;
    }

    private boolean hasHighSecurityGridFenceBlockAt(TileSecurityFenceHighCorner mainFence, int x, int y, int z) {
        return mainFence.worldObj.getBlock(x, y, z) instanceof BlockSecurityFenceHighGrid;
    }
        ...
        switch (side) {
            ...
            case 0:
                for (int i = 1; i < distance; i++) {
                    if (mainFence.hasHighSecurityCornerFenceBlockAt(mainFence, mainFence.xCoord, mainFence.yCoord, mainFence.zCoord + i))
            ...
            case 1:
                for (int i = 1; i < distance; i++) {
                    if (mainFence.hasHighSecurityCornerFenceBlockAt(mainFence, mainFence.xCoord - i, mainFence.yCoord, mainFence.zCoord))
    ...
    public TileSecurityFenceHighCorner getNextHighSecurityCornerFenceBlockDirectly(TileSecurityFenceHighCorner mainFence, int side, int distance) {
        ...
        switch (side) {
            /** South */
            case 0:
                if (mainFence.hasHighSecurityCornerFenceBlockAt(mainFence, mainFence.xCoord, mainFence.yCoord, mainFence.zCoord + distance))
                    tileEntity = mainFence.worldObj.getTileEntity(mainFence.xCoord, mainFence.yCoord, mainFence.zCoord + distance);
                break;
            /** West */
            case 1:
                if (mainFence.hasHighSecurityCornerFenceBlockAt(mainFence, mainFence.xCoord - distance, mainFence.yCoord, mainFence.zCoord))
    ...
    public int getFencePoleHeight(TileSecurityFenceHighCorner mainFence) {
        ...
        if (mainFence.worldObj != null) {
            while (this.hasHighSecurityPoleFenceBlockAt(mainFence, mainFence.xCoord, mainFence.yCoord + i + 1, mainFence.zCoord))
                i++;
        }
        return i;
    }
    ...
    public List<TileSecurityFenceHighPole> getAllFencePoles(TileSecurityFenceHighCorner mainFence) {
        ...
        if (i > 0) {
            List<TileSecurityFenceHighPole> fencePoles = new ArrayList();
            for (int j = 0; j < i; j++) {
                fencePoles.add((TileSecurityFenceHighPole) mainFence.worldObj.getTileEntity(mainFence.xCoord, mainFence.yCoord + i + 1, mainFence.zCoord));
            }
            return fencePoles;
        }
        return null;
    ...
    public void setGridsToAllFencePoles(TileSecurityFenceHighCorner mainFence) {
        ...
        for (int height = 0; height < i; height++) {
            for (int side = 0; side < 4; side++) {
                ...
                if (fence instanceof TileSecurityFenceHighPole)
        ...
        switch (side) {
            ...
            case 0:
                while (flag) {
                    ...
                    if (mainFence.hasHighSecurityBaseFenceBlockAt(mainFence, mainFence.xCoord, mainFence.yCoord, mainFence.zCoord + i + 1) && (metadata == 1 || metadata == 3)) {
                        i++;
                    } else {
                        flag = false;
                    }
            ...
            case 1:
                while (flag) {
                    ...
                    if (mainFence.hasHighSecurityBaseFenceBlockAt(mainFence, mainFence.xCoord - i - 1, mainFence.yCoord, mainFence.zCoord) && (metadata == 0 || metadata == 2)) {
                        i++;
                    } else {
                        flag = false;
                    }
    ...
    public boolean isBaseAtSideValid(TileSecurityFenceHighCorner mainFence, int side, int length) {
        ...
        switch (side) {
            /** South */
            case 0:
                return mainFence.hasHighSecurityCornerFenceBlockAt(mainFence, mainFence.xCoord, mainFence.yCoord, mainFence.zCoord + length);
            /** West */
            case 1:
                return mainFence.hasHighSecurityCornerFenceBlockAt(mainFence, mainFence.xCoord - length, mainFence.yCoord, mainFence.zCoord);
            /** North */
            case 2:
                return mainFence.hasHighSecurityCornerFenceBlockAt(mainFence, mainFence.xCoord, mainFence.yCoord, mainFence.zCoord - length);
            /** East */
    ...
    public boolean hasGridBetweenSpace(TileSecurityFenceHighCorner mainFence, int side, int length, int height) {
        switch (side) {
            ...
            case 0:
                for (int i = 0; i < height; i++) {
                    for (int j = 0; j < length; j++) {
                        if (!mainFence.hasHighSecurityGridFenceBlockAt(mainFence, mainFence.xCoord, mainFence.yCoord + i + 1, mainFence.zCoord + j + 1))
                            return false;
                    }
                }
                break;
            ...
            case 1:
                for (int i = 0; i < height; i++) {
                    for (int j = 0; j < length; j++) {
                        if (!mainFence.hasHighSecurityGridFenceBlockAt(mainFence, mainFence.xCoord - j - 1, mainFence.yCoord + i + 1, mainFence.zCoord))
                            return false;
                    }
                }
                break;
    ...
    private void buildFenceOff(TileSecurityFenceHighCorner mainFence1, TileSecurityFenceHighCorner mainFence2, int side, int length, int height) {
        // HERE Block grid = JCBlockRegistry.securityFenceHighGrid;
        switch (side) {
            /** South */
            case 0:
                for (int i = 0; i < height; i++) {
    ...
    private void changeFenceState(boolean turnOnOrOff, TileSecurityFenceHighCorner mainFence1, TileSecurityFenceHighCorner mainFence2, int side, int length, int height) {
        ...
        switch (side) {
            ...
            case 0:
                for (int i = 0; i < height; i++) {
                    for (int j = 0; j < length; j++) {
                        if (mainFence1.hasHighSecurityGridFenceBlockAt(mainFence1, mainFence1.xCoord, mainFence1.yCoord + i + 1, mainFence1.zCoord + j + 1)) {
            ...
            case 1:
                for (int i = 0; i < height; i++) {
                    for (int j = 0; j < length; j++) {
                        if (mainFence1.hasHighSecurityGridFenceBlockAt(mainFence1, mainFence1.xCoord - j - 1, mainFence1.yCoord + i + 1, mainFence1.zCoord)) {
    ...
    public HashMap<Integer, int[]> getAllFenceBlocks() {
        ...
        for (int i = -11; i < 12; i++) {
            for (int k = -11; k < 12; k++) {
                if (this.hasHighSecurityCornerFenceBlockAt(this, this.xCoord + i, this.yCoord, this.zCoord + k) || this.hasHighSecurityBaseFenceBlockAt(this, this.xCoord + i, this.yCoord, this.zCoord + k) || this.hasHighSecurityPoleFenceBlockAt(this, this.xCoord + i, this.yCoord, this.zCoord + k) || this.hasHighSecurityGridFenceBlockAt(this, this.xCoord + i, this.yCoord, this.zCoord + k)) {
